...
export const getChats = async (req: Request, res: Response) => {
  try {
    ...
    res.json({ message: "Error fetching chats" });
  }
};
...
export const getChattingUsers = async (req: Request, res: Response) => {
  try {
    ...
    if (!requester) {
      return res.json({ message: "Requesting user not found" });
    }
    ...
    const mappedUsers = await Promise.all(
      users.map(async (user) => {
        ...
        const lastMessage = await chatRepository
          .createQueryBuilder("chat")
          .leftJoinAndSelect("chat.sender", "sender")
          .where(
            "(chat.sender.id = :userId AND chat.receiver.id = :otherUserId) OR (chat.sender.id = :otherUserId AND chat.receiver.id = :userId)",
        ...
        return {
          ...
          passportUpload: user.isAdmin
            ? defaultAdminImageUrl
            : application?.passportUpload || defaultUserImageUrl,
          lastMessage: lastMessage ? lastMessage.message : "",
          lastMessageCreatedAt: lastMessage
            ? lastMessage.createdAt.toISOString()
            : null,
          // Correctly handle potential undefined sender
          lastMessageSentByCurrentUser: lastMessage
            ? lastMessage.sender
    ...
    res.json({ message: "Error fetching chatting users with last message" });
  }
};
...
export const deleteChatMessage = async (req: Request, res: Response) => {
  try {
    ...
    if (!chatMessage) {
      return res.json({ message: "Chat message not found" });
    }
    ...
    res.json({ message: "Error deleting chat message" });
  }
};
